...
const VideoPlayerWithChaptersAndTranscript = forwardRef ( ( { videoData, className }, ref) => {
  ...
  return (
      <div
        className={`video-container order-1 lg:mx-auto my-3 lg:my-5 lg:mb-8 ${
          (!videoData.chapters || videoData.chapters.length === 0)
            ? "mx-auto lg:w-[718px] md:mb-8 lg:mt-7"
      ...
      >
        ...
        <div
          className={`order-3 lg:order-2 mt-5 lg:mt-0 lg:ml-2px mb-8 lg:mb-0 leading-4 text-[14px] cursor-pointer`}
        >
        ...
        {
          (typeof videoData.transcript !== 'undefined') && 
          <VideoTranscript transcript={videoData.transcript} />
        }
      </div>
  );
});

export default VideoPlayerWithChaptersAndTranscript;
