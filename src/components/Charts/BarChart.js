...
const BarChart = ({ percentage, children }) => {
  ...
  useEffect(() => {
    ...
    bars.forEach((bar, index) => {
      bar.style.animationDelay = `${index * 0.085}s`;
    });
    ...
    <StyledChart >
      ...
      <div className="chart-container md:flex md:justify-between w-full">
        ...
        {PulmonaryData.map((item, index) => (
          <div key={index} className={`scroll-${index}`}>
            <h3 className="font-bold text-center mt-3 mb-5">{item.followupYear}-year follow-up</h3>
            <ul className="bars-container md:flex md:flex-row">
            {item.data.map((childItem, childIndex) => (
              <li key={childIndex} className="md:flex md:flex-col-reverse">
                {childItem.n}
                <div className={`bar rounded-[8px] h-[43px] flex items-center `}>
                  <div
                    className={`mr-1 rounded-[8px] bar-${childIndex + index*4}` }
                    style={{
            ...
            <div className="percent-increments md:hidden flex w-full justify-between mt-6">{percentIncrements}</div>
            <p className="md:hidden text-center font-bold mt-3 mb-7">
...
const StyledChart = styled.div`
  ...
  .bar {
    ...
    div { // Chart Data
      ...
      &.animate ~ span {
        animation: revealing-numbers .5s ease 1 forwards;
      }
    }
    div.bar-0 {
      background: ${CSS.COLORS.heatedGold};
      & ~ span { animation-delay: 1s;}
    }
    div.bar-1 {
      background: ${CSS.COLORS.bakedCoral};
      animation-delay: .15s;
      & ~ span { animation-delay: 1.05s;}
    }
    div.bar-2 {
      background: ${CSS.COLORS.blazingRose};
      animation-delay: .30s;
      & ~ span { animation-delay: 1.1s;}
    }
    div.bar-3 {
      background: ${CSS.COLORS.burnBrick};
      animation-delay: .45s;
      & ~ span { animation-delay: 1.15s;}
    }    
    div.bar-4 {
      background: ${CSS.COLORS.heatedGold};
      animation-delay: .60s;
      & ~ span { animation-delay: 1.20s;}
    }
    div.bar-5 {
      background: ${CSS.COLORS.bakedCoral};
      animation-delay: .75s;
      & ~ span { animation-delay: 1.25s;}
    }
    div.bar-6 {
      background: ${CSS.COLORS.blazingRose};
      animation-delay: .90s;
      & ~ span { animation-delay: 1.30s;}
    }
    div.bar-7 {
      background: ${CSS.COLORS.burnBrick};
      animation-delay: 1.05s;
      & ~ span { animation-delay: 1.35s;}
    }      
  }
  span.inside {
    @media screen and (max-width: 767px) {
